...
function ImageUploadZone({
  label, badge, badgeClass, imageUrl, uploading, error, onUpload, onRemove,
      ...
      <div
        className={cn(
          ...
          imageUrl
            ...
            : error
            ? 'border-destructive bg-destructive/3'
...
export default function AddInspirationPage() {
  ...
  // ── Upload handler — uses dedicated /api/upload/inspiration-image endpoint ──
  const uploadImage = async (file: File, type: 'before' | 'after') => {
    ...
    try {
      ...
      if (data.success && data.url) {
        ...
        toast({
          title: '✅ Uploaded',
          description: `${type === 'before' ? 'Before' : 'After'} image ready`,
        });
      } else {
  ...
  const validateForm = () => {
    ...
    // Guard: both images must be different
    if (
      formData.beforeImage &&
      formData.afterImage &&
      formData.beforeImage === formData.afterImage
    ) {
      e.afterImage = 'After image must be different from the before image';
    }
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gradient-to-br from-secondary/5 to-secondary/[0.02] rounded-2xl border border-secondary/15 p-5 sm:p-6 space-y-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-0.5">
                    Upload two <span className="font-medium">different</span> images showing the transformation
                  </p>
                </div>
                ...
                {formData.beforeImage &&
                  ...
                  <div className="flex items-center gap-2 p-3 rounded-xl bg-destructive/10 border border-destructive/20 text-sm text-destructive">
                    ...
                    Both images are the same — please upload a different after photo.
                  </div>
                )}
              ...
              <div className="bg-gradient-to-br from-primary/5 to-primary/[0.02] rounded-2xl border border-primary/15 p-5 sm:p-6 space-y-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-0.5">
                      Help clients find your work — select up to 5
                    </p>
                  </div>
              ...
              <div className="bg-gradient-to-br from-secondary/5 to-secondary/[0.02] rounded-2xl border border-secondary/15 p-5 sm:p-6 space-y-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-0.5">
                    Extra context helps clients connect with your work
                  </p>
                </div>
